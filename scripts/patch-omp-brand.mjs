 ...
 *   D. 插件面板命令提示 "omp plugin install …"（2 处）→ 插值 APP_NAME
 *   E. 更新提示与示例 "omp update"/"--check"/"--canary"（4 处）→ 插值
 *   F. 模型错误提示 Run "omp models" to see（3 处）→ 插值
 *   G. 审批摘要 "omp wants to run <tool>" → 插值
 *
...
for (const file of targets) {
	...
	// ── D. 插件面板命令提示 ──
	const panels = [
		[`"Install npm plugins:        omp plugin install <package>"`, "`Install npm plugins:        ${" + V + "} plugin install <package>`"],
		[`"Install marketplace plugins: omp plugin install <name>@<marketplace>"`, "`Install marketplace plugins: ${" + V + "} plugin install <name>@<marketplace>`"],
	];
	let dHit = 0;
	for (const [from, to] of panels) {
		if (src.includes(from)) { src = src.split(from).join(to); dHit++; }
	}
	log.push(dHit === panels.length ? "✓ D. 插件面板提示（2 处）" : dHit > 0 ? `✓ D. 插件面板提示（${dHit}/2 处，其余已打）` : "⏭ D. 插件面板提示（已打）");

	// ── E. omp update 字面量（通知命令 + examples 数组）──
	const upRe = /"omp update([^"]*)"/g;
	const upHits = (src.match(upRe) || []).length;
	if (upHits > 0) {
		src = src.replace(upRe, "`${" + V + "} update$1`");
		log.push(`✓ E. update 提示（${upHits} 处）`);
	} else log.push("⏭ E. update 提示（已打）");

	// ── F. 模型错误提示 ──
	const fFrom = `Run "omp models" to see`;
	const fTo = `Run "\${${V}} models" to see`;
