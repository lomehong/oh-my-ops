...
export function registerShellTools(pi: ExtensionAPI, ctx: OpsContext): void {
	const z = pi.zod;
	...
	registerOpsTool(pi, {
		...
		description:
			...
			"高危命令会被第②层内容硬拒拦截。输出超 3000 行/50KB 时由宿主截断并存入 artifact。",
		parameters: z.object({
		...
		async execute(_toolCallId, params, signal) {
			...
			assertAuthorized("ops_shell_exec", { command: cmd }, ctx.authzView);
			const result = await ctx.shell.exec(["sh", "-c", cmd], { timeoutMs: timeout * 1000, signal });
			return {
				content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
				details: { authz: ctx.lastAuthzSource() },
			};
		},
	});
	...
	registerOpsTool(pi, {
		...
		approval: EXEC,
		description:
			"执行一段 shell 脚本（多行命令）。脚本通过 sh -c 解析。输出超 3000 行/50KB 时由宿主截断。",
		parameters: z.object({
		...
		async execute(_toolCallId, params, signal) {
			...
			assertAuthorized("ops_shell_exec", { command: script }, ctx.authzView);
			const result = await ctx.shell.exec(["sh", "-c", script], { timeoutMs: timeout * 1000, signal });
			return {
				content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
				details: { authz: ctx.lastAuthzSource() },
			};
