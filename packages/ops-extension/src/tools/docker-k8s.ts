import { READ, EXEC, tierOf } from "@ops-pi/core";
...
export function registerDockerTools(pi: ExtensionAPI, ctx: OpsContext): void {
	const z = pi.zod;
	...
	registerOpsTool(pi, {
		...
		async execute(_id, params, signal) {
			...
			assertAuthorized("ops_docker_ps", { host: undefined }, ctx.authzView);
			...
			return { content: [{ type: "text", text: result.stdout || "(no containers)" }], details: { authz: "read" } };
		},
	});
	...
	registerOpsTool(pi, {
		...
		async execute(_id, params, signal) {
			...
			assertAuthorized("ops_docker_logs", { host: container }, ctx.authzView);
			const result = await ctx.shell.exec(["docker", "logs", "--tail", String(lines), container], { signal, timeoutMs: 15_000 });
			return { content: [{ type: "text", text: result.stdout || result.stderr }], details: { authz: "read" } };
		},
	});
	...
	registerOpsTool(pi, {
		...
		loadMode: "essential",
		approval: EXEC,
		description: "在容器内执行命令（exec 档，需 Owner 预授权）。",
		parameters: z.object({
			container: z.string().describe("容器名或 ID"),
		...
		async execute(_id, params, signal) {
			...
			assertAuthorized("ops_docker_exec", { host: container, command }, ctx.authzView);
			const result = await ctx.shell.exec(["docker", "exec", container, "sh", "-c", command], { signal, timeoutMs: 30_000 });
			return { content: [{ type: "text", text: result.stdout || result.stderr }], details: { authz: ctx.lastAuthzSource() } };
		},
	});
	...
	registerOpsTool(pi, {
		...
		description: "Docker Compose 操作。ps/logs 为只读；up/down/restart 为变更类（需 Owner 预授权）。",
		parameters: z.object({
		...
		async execute(_id, params, signal) {
			...
			assertAuthorized("ops_docker_compose", { host: dir, service: action, action }, ctx.authzView);
			const result = await ctx.shell.exec(["docker", "compose", "-f", `${dir}/docker-compose.yml`, action], { signal, timeoutMs: 60_000 });
			return { content: [{ type: "text", text: result.stdout || result.stderr || `exit=${result.exitCode}` }], details: { authz: ctx.lastAuthzSource() } };
		},
	});
}

/** P3：K8s 工具——kubectl CLI（控制节点执行，靠 kubeconfig context 切换集群，方案 §3.6） */
export function registerK8sTools(pi: ExtensionAPI, ctx: OpsContext): void {
	const z = pi.zod;
	...
	registerOpsTool(pi, {
		...
		async execute(_id, params, signal) {
			const p = params as Record<string, unknown>;
			assertAuthorized("ops_k8s_pods", { host: undefined }, ctx.authzView);
			...
			return { content: [{ type: "text", text: result.stdout || "(no pods)" }], details: { authz: "read" } };
		},
	});
	...
	registerOpsTool(pi, {
		...
		async execute(_id, params, signal) {
			...
			const ns = p.namespace !== undefined ? String(p.namespace) : "default";
			assertAuthorized("ops_k8s_logs", { host: pod }, ctx.authzView);
	...
	registerOpsTool(pi, {
		...
		description: "K8s Rollout 操作。status 为只读；restart/undo 为变更类（需 Owner 预授权）。",
		parameters: z.object({
		...
		async execute(_id, params, signal) {
			...
			const ns = p.namespace !== undefined ? String(p.namespace) : "default";
			assertAuthorized("ops_k8s_rollout", { host: name, service: ns, action }, ctx.authzView);
			...
			return { content: [{ type: "text", text: result.stdout || `exit=${result.exitCode}` }], details: { authz: ctx.lastAuthzSource() } };
		},
	});
	...
	registerOpsTool(pi, {
		...
		loadMode: "essential",
		approval: EXEC,
		description: "在 K8s Pod 容器内执行命令（exec 档，需 Owner 预授权）。",
		parameters: z.object({
			pod: z.string().describe("Pod 名称"),
			command: z.string().describe("要执行的命令"),
		...
		async execute(_id, params, signal) {
			...
			assertAuthorized("ops_k8s_exec", { host: pod, command }, ctx.authzView);
