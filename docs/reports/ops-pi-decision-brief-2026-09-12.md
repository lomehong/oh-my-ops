...
> 一句话说清：omo 的**验收标准**是什么？（例如"能在无人值守下完成只读巡检并对指定服务执行重启，全过程留审计"）
...
**A（推荐）**：你指定 omo 代码的目标仓（本地路径或远端地址）。我在那里建 `docs/designs/`、`docs/reports/`、`docs/tasks/`，并落 `scripts/task-ledger.mjs`。
**B**：暂不 git 化，只产出文档（方案 + 报告 + 需求包）。**代价**：违反"一切知识进结构""提交前工作区干净"两条纪律，无法用台账承载任务痕迹。
**C**：`git init /workspace` 作为 omo 起点（若这个项目本来就该在这里）。
...
> omo 代码放哪？（路径 / 远端仓 / 或"还没定，先 git init /workspace"）
