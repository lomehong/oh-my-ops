...
| 影响谁？ | 服务机运维（受益）；4 台已接入实例（无感，/enroll、/healthz 不动）；Gitea（仅既有 API 复用）；**yufu 网关**（/ui 平面的 TLS 终结 + 认证 + 角色权限的上游） |
| 如何验证？ | 新增契约探针 probe-kb-ui.sh（登录→读态→签码→改配置→预检→重启切换全链）+ 单测 + test:ci 全绿回归 |
| 还有什么没有确认？ | 2 项：yufu 身份头键名（一期用可配默认值兜底，不阻断）；实例通道是否二期迁 yufu（候选，不动生产） |
...
| 5 | 认证鉴权由 yufu 负责（服务部署在 yufu 之后，TLS 天然由 yufu 终结） | **网关信任模式**：`/ui` 全平面要求 yufu 注入的**身份头**（键名 `OMO_KB_UI_IDENTITY_HEADER`，默认 `X-Forwarded-User`，以 yufu 规格为准）；缺失/为空 ⇒ 401（直连 8787 绕过网关不可达 UI）；审计 actor=身份头值（按人区分天然成立） | 主人澄清 2026-09-21 |
| 6 | 同端口 8787（服务监听不变） | 服务仍单进程监听 8787；实例直连 `/enroll`（服务自签证书通道，不变），管理后台经 yufu 反代到 `/ui`（TLS 由 yufu 终结）。页面内全部相对路径，兼容 yufu 子路径前缀 | 主人决策+澄清 2026-09-21 |
...
| 2 | yufu 深度集成（角色→页面能力映射、yufu 侧单点登出等） | 本期只做「身份头门禁」；角色/权限分解完全由 yufu 承担（主人定），服务不重复造权限模型 |
| 3 | enroll 协议/实例侧任何变更 | 兼容红线：4 台生产实例零感 |
| 4 | 前端框架与构建链 | 服务包「零外部依赖」既有约束（发布清单断言佐证）；页面规模（4 区块）不需要框架 |
| 5 | TLS 证书**自签生成**进 UI（「签发」指证书，非 enroll 码——码签发在 UI 内，见 Do 3） | 实例直连通道的自签证书生成属安装器职责（openssl）；UI 平面 TLS 由 yufu 终结，服务无需额外证书 |

### To Confirm（待确认）

| # | 待确认项 | 问谁 | 状态 |
|---|---|---|---|
| 1 | yufu 身份头**键名**与子路径挂载形态（规格在 `E:\code\go\Yufu`，本容器不可达；一期先按 `X-Forwarded-User` 实现并做成可配） | 主人 | 一期可开工（键名可配兜底），对接前确认 |
| 2 | ~~操作者身份粒度~~ **已解决**：actor=yufu 身份头，天然按人 | — | 已闭环 |
| 3 | 实例 enroll 通道二期是否也迁到 yufu 后面（合法证书替代自签，消灭 --allow-insecure-tls；需改 4 台实例 enroll 地址） | 主人 | 二期候选，一期不动生产实例 |
...
- 范围外参与方：Gitea（twin.hzins.com，只被既有 API 复用）；logstash-124 等 4 实例（不改协议）；**yufu 网关**（上游反代+认证+角色权限，部署形态由主人侧保障）。
- UI 前端纪律：所有静态资源与 API 调用用**相对路径**（兼容 yufu 任意子路径前缀挂载）；页面不引用任何外部 CDN 资源。
...
| Security 安全 | 有（核心） | ① **身份头门禁**：/ui 全平面要求 yufu 身份头，缺失 ⇒ 401（直连绕网关不可入）；服务只信反代头 ⇒ 部署上必须保证 8787 不对实例网段外的来源开放 /ui（yufu 与服务同机/内网，主人侧部署保障）；② `OMO_KB_UI` 仍默认 off（升级即安全态，部署对接 yufu 后显式开）；③ 会话仅存身份快照（HttpOnly cookie，8h）；④ 写操作校验 Origin + 失败复用 throttled(ip) 限流；⑤ **一切响应不含秘密**（admin 凭据只回「已配置(sha1 前 8 位)」）；⑥ 审计 actor=yufu 身份头值，按人可追责 |
...
| # | 类型 | 描述 | 处置 |
|---|---|---|---|
| 1 | Unknown | yufu 身份头键名/子路径形态（规格未提供） | 一期按 `X-Forwarded-User` 实现 + 键名可配（`OMO_KB_UI_IDENTITY_HEADER`）兜底；对接前与主人核对 |
| 2 | Unknown | 实例通道是否二期迁 yufu（合法证书替代自签） | 已登记为二期候选（待确认 #3），一期生产实例零动 |
| 3 | Human Decision | 认证/角色权限归属 | **主人已拍板**（2026-09-21 两次澄清：全归 yufu，服务在其后部署，TLS 由 yufu 终结）；工程落实=身份头门禁+UI 默认 off+限流+Origin 校验+秘密不回显，记录于 §8 |
...
| High-risk Change（二次澄清：认证/角色/TLS 归属） | 服务部署在 yufu 之后：TLS 由 yufu 终结、认证与角色权限由 yufu 负责；/ui 平面改**身份头门禁**（替代原 none+默认关双保险中的"无认证"态）；enroll 码签发确认在管理后台内（证书自签生成仍属安装器） | 主人 2026-09-21 |
