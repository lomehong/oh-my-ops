...
| 连接行为 | close `4008/4009` 后不再自动重连 | 中。**4008 真机触发源=Hub 管理员踢线**（§3.6）；`4009` 上游已标注「已废、Hub 不再主动发」，保留分支用于兼容旧 Hub。置 `closed=true` 后**实例内无自动恢复路径**（进程内无外部接口），需宿主重启或人工重新 `connect()` → 管理员误踢的代价由「反复重连」变成「人工重启」，与契约要求一致但需运维知情 |
